...
import { ChevronLeft, ChevronRight, ChevronDown, CheckCircle, XCircle, Clock, TrendingUp } from 'lucide-react';
...
export function AttendanceCalendar({ records, overallPercentage, label = 'Attendance' }: AttendanceCalendarProps) {
  ...
  // Check if next month is in the future beyond current month
          ...
          >
            <Card className="shadow-sm border-l-4" style={{
              borderLeftColor: selectedRecord === 'present' ? 'hsl(var(--success))' :
                selectedRecord === 'absent' ? 'hsl(var(--destructive))' : 'hsl(var(--warning))'
            }}>
              <CardContent className="p-3.5 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    {selectedRecord === 'present' ? 'Attended school' :
                      selectedRecord === 'absent' ? 'Did not attend' : 'Arrived late'}
                  </p>
                </div>

      ...
      <div className="p-4 space-y-3">
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button className="w-full"><Plus className="w-4 h-4 mr-2" /> Raise a Query</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>New Query</DialogTitle></DialogHeader>
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <Label>Subject</Label>
                <Input placeholder="Brief subject" value={form.subject} onChange={e => setForm(p => ({ ...p, subject: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Description</Label>
                <Textarea placeholder="Describe your query..." value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className="min-h-[80px]" />
              </div>
              ...
              <Button className="w-full" onClick={handleSubmit} disabled={submitQuery.isPending || !form.subject || !form.description}>
        ...
        {isLoading ? (
          ...
          queries?.map(q => (
            <Card key={q.id} className="cursor-pointer" onClick={() => { setSelected(q); setResponseText(''); }}>
            ...
            <div className="space-y-3">
                ...
                <div className="border-t pt-3 space-y-2">
                  ...
                  {responses.map(r => (
                    <div key={r.id} className="bg-muted/50 rounded-lg p-3">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{r.responder_name} • {format(new Date(r.created_at), 'dd MMM yyyy')}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {selected.status !== 'closed' && selected.status !== 'resolved' && (
                <div className="flex gap-2">
                  <Textarea placeholder="Add a reply..." value={responseText} onChange={e => setResponseText(e.target.value)} className="min-h-[50px]" />
                  <Button size="icon" className="shrink-0 self-end" onClick={handleRespond} disabled={respondToQuery.isPending || !responseText.trim()}>

...
import { useTeacherOnlineClasses } from '@/hooks/useOnlineClasses';
...
export default function TeacherOnlineClasses() {
  const { user } = useAuth();
  const { data: classes = [], isLoading } = useTeacherOnlineClasses(user?.id);
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">View and start your scheduled virtual classes</p>
        </div>
        ...
        {isLoading ? <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div> : classes.length === 0 ? (
          <EmptyState icon={Video} title="No online classes" description="You don't have any scheduled online classes yet." />
        ) : (
          <>
            {upcoming.length > 0 && (
